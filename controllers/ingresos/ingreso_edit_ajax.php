...
function ingresoEditAjaxController() {
    global $con;
    ...
    if (!verifyCSRFToken($csrf_token)) {
        error_log('[SECURITY] CSRF token inválido en ingreso edit para usuario: ' . getCurrentUsername());
        echo json_encode(['success' => false, 'message' => 'Token de seguridad inválido']);
        exit;
    }
    
    // Validar datos requeridos
    if(!isset($_POST['id']) || !isset($_POST['fecha']) || !isset($_POST['importe']) || !isset($_POST['period_type_id'])) {
    ...
    try {
        ...
        $result = $ingresoModel->updateIngreso($id, $fecha, $importe, $period_type_id, $descripcion);
