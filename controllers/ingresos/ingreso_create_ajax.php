...
function ingresoCreateAjaxController() {
    global $con;
    ...
    if (!verifyCSRFToken($csrf_token)) {
        error_log('[SECURITY] CSRF token inválido en ingreso create para usuario: ' . getCurrentUsername());
    ...
    try {
        ...
        $result = $ingresoModel->createIngreso($fecha, $importe, $period_type_id, $descripcion);
        
        if($result) {
            echo json_encode(['success' => true, 'message' => 'Ingreso creado correctamente']);
        } else {
            echo json_encode(['success' => false, 'message' => 'Error al crear el ingreso']);
        }
    } catch (Exception $e) {
